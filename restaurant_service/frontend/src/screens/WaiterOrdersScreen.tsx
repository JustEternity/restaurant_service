...
const WaiterOrders = () => {
  ...
  const updatePlateStatus = async (plateId: number, newStatus: string) => {
    try {
      const response = await fetch(`${API_CONFIG.BASE_URL}/orders/plate/${plateId}/status/${newStatus}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      ...
      if (selectedOrder) {
        const updatedPlates = selectedOrder.plates.map(plate =>
          plate.id === plateId ? { ...plate, cooking_status: newStatus } : plate
        );
        setSelectedOrder({ ...selectedOrder, plates: updatedPlates });
        setOrders(prev =>
          prev.map(order =>
            order.id === selectedOrder.id ? { ...order, plates: updatedPlates } : order
          )
        );
      }
      Alert.alert('Успех', `Статус блюда изменён на "${newStatus}"`);
    } catch (error) {
  ...
  const renderPlateItem = (plate: PlateInOrder) => (
    <View style={styles.plateItem}>
      ...
      <View style={styles.plateStatusContainer}>
        ...
        {selectedOrder?.status === 'active' && plate.cooking_status !== 'served' && (
          <TouchableOpacity
            style={styles.statusButton}
            onPress={() => {
              Alert.alert(
                'Изменить статус',
                `Выберите новый статус для "${plate.plate_name}"`,
                [
                  { text: 'Отмена', style: 'cancel' },
                  ...(plate.cooking_status === 'waiting' ? [
                    { text: 'Готовится', onPress: () => updatePlateStatus(plate.id, 'preparing') }
                  ] : []),
                  ...(plate.cooking_status === 'preparing' ? [
                    { text: 'Готово', onPress: () => updatePlateStatus(plate.id, 'ready') }
                  ] : []),
                  ...(plate.cooking_status === 'ready' ? [
                    { text: 'Подано', onPress: () => updatePlateStatus(plate.id, 'served') }
                  ] : []),
                ]
              );
            }}
